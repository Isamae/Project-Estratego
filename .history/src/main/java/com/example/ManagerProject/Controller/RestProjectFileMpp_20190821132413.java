            ...
            try {
                ...
                for(int j = 0 ; j < predecesoras.length; j++){
                    if(predecesoras[j].length() < 8){}
                        ...
                        //RelationType type = ;
                        if(lag.contains("d")){
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("d", "")),TimeUnit.DAYS));
                        }
                        else if(lag.contains("h")){
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("h", "")),TimeUnit.HOURS));
                        }
                        else if(lag.contains("y")){
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("y", "")),TimeUnit.YEARS));
                        }
                        else if(lag.contains("w")){
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("w", "")),TimeUnit.WEEKS));
                        }
                        else if(lag.contains("m")){
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("m", "")),TimeUnit.MONTHS));
                        }
                        else{
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("p", "")),TimeUnit.PERCENT));
                        }
