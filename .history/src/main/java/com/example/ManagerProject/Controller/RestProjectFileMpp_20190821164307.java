                ...
                for(int j = 0 ; j < predecesoras.length; j++){
                    if(predecesoras[j].length() < 8){}
                        ...
                        if(lag.contains("d")){
                            ...
                            task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.START_FINISH,  Duration.getInstance(Double.parseDouble(lag.replace("h", "")),TimeUnit.HOURS));
                        }
                        else if(lag.contains("y")){
                            task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.START_FINISH,  Duration.getInstance(Double.parseDouble(lag.replace("y", "")),TimeUnit.YEARS));
                        }
                        else if(lag.contains("w")){
                            task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.START_FINISH,  Duration.getInstance(Double.parseDouble(lag.replace("w", "")),TimeUnit.WEEKS));
                        }
                        else if(lag.contains("m")){
                            task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.START_FINISH,  Duration.getInstance(Double.parseDouble(lag.replace("m", "")),TimeUnit.MONTHS));
                        }
                        else{
                            task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.START_FINISH,  Duration.getInstance(Double.parseDouble(lag.replace("p", "")),TimeUnit.PERCENT));
                        }
                    }
