    ...
    public static ProjectFile addAsignacionesRecursos(ProjectFile project,JSONObject jsonObject) throws JSONException, ParseException {
        ...
        for(int i=0 ;i< ((JSONArray)(jsonObject.get("asigRecursos"))).length();i++){
            try {
                ...
                assignment.setActualStart(df.parse(jsonObject.getString("ActualStart")));
                assignment.setActualFinish(df.parse(jsonObject.getString("ActualFinish")));
                assignment.setStart(df.parse(jsonObject.getString("Start")));
                assignment.setFinish(df.parse(jsonObject.getString("Finish")));
                assignment.setUnits(df.parse(jsonObject.getString("Units")));
            } catch (JSONException e) {
                e.printStackTrace();
            } catch (ClassCastException e) {
                e.printStackTrace();
