        ...
        for(int i=0 ;i< ((JSONArray)(jsonObject.get("asigRecursos"))).length();i++){
            try {
                ...
                if(json.getString("ActualWork").contains("d")){
                    ...
                    assignment.setActualWork(Duration.getInstance(Double.parseDouble(json.getInt("ActualWork").replace("d", ""),TimeUnit.DAYS));

                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("h", "")),TimeUnit.HOURS));
                }
                else if(json.getString("ActualWork").contains("y")){
                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("y", "")),TimeUnit.YEARS));
                }
                else if(json.getString("ActualWork").contains("w")){
                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("w", "")),TimeUnit.WEEKS));
                }
                else if(json.getString("ActualWork").contains("m")){
                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("m", "")),TimeUnit.MINUTES));
                }
                else if(json.getString("ActualWork").contains("M")){
                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("M", "")),TimeUnit.MONTHS));
                }
                else{
                    task.addPredecessor( project.getTaskByID(Integer.parseInt(idH)), relationType,  Duration.getInstance(Double.parseDouble(json.getString("ActualWork").replace("p", "")),TimeUnit.PERCENT));
                }
