                ...
                for(int j = 0 ; j < predecesoras.length; j++){
                    if(predecesoras[j].length() < 8){}
                        ...
                        if(lag.contains("d")){
                            ...
                            project.getTaskByID(json.getInt("id")).addPredecessor( project.getTaskByID(Integer.parseInt(idH)), RelationType.valueOf(tipo),  Duration.getInstance(Double.parseDouble(lag.replace("m", "")),TimeUnit.WEEKS));
                        }
                        else{
