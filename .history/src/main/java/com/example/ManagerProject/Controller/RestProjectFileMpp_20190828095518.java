...
public class RestProjectFileMpp {
    ...
    public static ProjectFile addCamposPersonalizados(ProjectFile project,JSONObject jsonObject) throws JSONException{
        ...
        for(int i=0 ;i< ((JSONArray)(jsonObject.get("CamposPersonalizados"))).length();i++){
            try {
                ...
                if(object.getString("Alias Campo") == null){}
                else{
                    FieldType fieldType = FieldTypeHelper.getInstance14(object.getInt("FieldTypeValue")); 
                    ...
                    project.getCustomFields().getCustomField(fieldType);
                    project.getCustomFields().registerValue(item);
                    project.getTables().get(0).getColumns().get(0).setFieldType(fieldType);
    ...
    public static ProjectFile addColumnas(ProjectFile project,JSONObject jsonObject) throws Exception
    {
        ...
        Iterator<Column> iter = project.getTables().get(0).getColumns().iterator();
        while(iter.hasNext()){
        ...
        for(int i = 0; i < json.length(); i ++ ){
            ...
            FieldType fieldType = FieldTypeHelper.getInstance14(json.getJSONObject(i).getInt("FieldTypeValue"));
            column.setFieldType(fieldType);
            column.setTitle(json.getJSONObject(i).getInt("ColumTitulo"));
        }
