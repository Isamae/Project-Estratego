    ...
    public static ProjectFile addTarea(ProjectFile project,JSONObject jsonObject) throws JSONException {
        ...
        for(int i=0;i< ((JSONArray)(jsonObject.get("tareas"))).length();i++){
            try {
                ...
                String[] predecesoras = jsonObject3.getString("Predecessors").split(",");
                for(int j = 0 ; j < predecesoras.length; j++){
                    if(predecesoras[j].length() < 14){}
                    else{
                        System.out.println(predecesoras[j]);
                        String[] datos = predecesoras[j].split("->");

                        String lag = datos[0].substring(15, 20);
                        String tipo = datos[0].substring(26, 28);
                        //String id = datos[1].substring(10, 14);
                        System.out.println("lag:"+lag+"tipo:"+tipo+"id:");
                    }
                    
                }
                //project.getTaskByID(json.getInt("id"));

                //project.getTaskByID(json.getInt("id")).addPredecessor(targetTask, type, lag));

            

                //System.out.println( "fIN ACTUAL" + project.getTaskByID(json.getInt("id")).getActualFinish());

                //System.out.println("Iniciio "+  project.getTaskByID(json.getInt("id")).getStartText());
