...
public class RestProjectFileMpp {
    ...
    public static ProjectFile addTarea(ProjectFile project,JSONObject jsonObject) throws JSONException, ParseException {
        ...
        if(jsonObject.getString("FinishDate")!="null"){
            ...
            project.getFinishDate().setTime(df.parse(jsonObject.getString("StartDate")).getTime());
            project.getFinishDate().setSeconds(df.parse(jsonObject.getString("StartDate")).getSeconds());
            project.getFinishDate().setMinutes(df.parse(jsonObject.getString("StartDate")).getMinutes());
            project.getFinishDate().setHours(df.parse(jsonObject.getString("StartDate")).getHours());
            project.getFinishDate().setMonth(df.parse(jsonObject.getString("StartDate")).getMonth());
            project.getFinishDate().setYear(df.parse(jsonObject.getString("StartDate")).getYear());
        }
        if(jsonObject.getString("PStartDate")!="null"){
            df.parse(jsonObject.getString("PStartDate"));
