    ...
    public String postJson(@RequestBody String payload) throws Exception{
        ...
        for(int i=0;i< ((JSONArray)(jsonObject.get("recursos"))).length();i++){
            try {
                ...
                (projectObj.getTaskByID(1)).setName(json.getString("name"));
                (projectObj.getTaskByID(1)).setUniqueID(json.getInt("uniqueID"));
                (projectObj.getTaskByID(1)).setActive(json.getBoolean("estado"));
