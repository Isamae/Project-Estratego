    ...
    public static ProjectFile addTarea(ProjectFile project,JSONObject jsonObject) throws JSONException, ParseException {
        ...
        for(int i=0;i< ((JSONArray)(jsonObject.get("tareas"))).length();i++){
            try {
                ...
                String duracion = jsonObject3.getString("duracion");
                Duration value;
                
                if(duracion.contains("h")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("h", "")), TimeUnit.HOURS);
                }

                else if(duracion.contains("s")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("s", "")), TimeUnit.MINUTES);
                }

                else if(duracion.contains("d")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("d", "")), TimeUnit.DAYS);
                }
                else if(duracion.contains("w")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("w", "")), TimeUnit.WEEKS);
                }

                else if(duracion.contains("M")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("M", "")), TimeUnit.MONTHS);
                }
                else if(duracion.contains("y")){
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("y", "")), TimeUnit.YEARS);
                }
                else{
                    value = Duration.getInstance(Integer.parseInt(duracion.replace("p", "")), TimeUnit.PERCENT);
                }
                ...
                project.getTaskByID(json.getInt("id")).setDuration(10, value);
